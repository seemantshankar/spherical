...
// mockVectorAdapter provides deterministic vector results for fallback.
type mockVectorAdapter struct {
	results []retrieval.VectorResult
	called  bool
}

func (m *mockVectorAdapter) Search(ctx context.Context, query []float32, k int, filters retrieval.VectorFilters) ([]retrieval.VectorResult, error) {
	m.called = true
	return m.results, nil
}
func (m *mockVectorAdapter) Insert(ctx context.Context, vectors []retrieval.VectorEntry) error {
	return nil
}
...
// T016: fallback when keyword returns nothing; semantic spec_fact embedding supplies fact with explanation/provenance.
func TestStructuredRetrieval_SemanticFallbackReturnsExplanation(t *testing.T) {
	tenantID := uuid.New()
	...
	assert.Equal(t, "Battery range is 300 km.", resp.StructuredFacts[0].Explanation)
}
