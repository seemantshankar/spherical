...
3. **Given** the admin provides only a brochure PDF path, **When** the ingestion CLI runs, **Then** it automatically invokes the existing `pdf-extractor` CLI from feature `001-pdf-spec-extractor`, captures the generated Markdown, and feeds it into the normalization pipeline without any manual copy steps.
...
> **Scope Note**: This release exposes lineage/drift insights through CLI commands plus REST/GraphQL/gRPC endpoints that an existing analytics dashboard can consume. Building a bespoke UI is explicitly deferred and will be addressed in a later feature.
